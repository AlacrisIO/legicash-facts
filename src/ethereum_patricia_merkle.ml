...
module Key : sig
  ...
  module Unpacked = struct
    ...
    let split key =
      if key = empty_key then
        raise (Internal_error "next_key: got empty key");
      (Char.code key.[0],drop 1 key)

    (* remove terminator, if any, from key *)
    let remove_terminator key =
      let len = String.length key in
      if key.[len-1] = nybble_terminator then
        String.sub key 0 (len - 1)
      else
        key
          ...
          (fun ndx c ->
            ...
            if ndx = len - 1 then (
              if c_val > nybble_terminator_val then
                raise (Internal_error "Unpacked.of_string: final character must be less than or equal to nybble terminator")
            )
            else
              if c_val > 0x0F then
                raise (Internal_error "Unpacked.of_string: character must be less than or equal to 0x0F"))
          s
      in
      s
  ...
  let pack unpacked =
    ...
    let (nybbles0,flags0) =
      if len > 0 && unpacked.[len - 1] = nybble_terminator then
        (String.sub unpacked 0 (len - 1),2)
      else
        (unpacked,0)
    in
    ...
    let nybbles =
      if odd_len = 0 then
        String.make 1 (Char.chr flags) ^ "\x00" ^ nybbles0
      else
        String.make 1 (Char.chr flags) ^ nybbles0
    in
    String.init ((String.length nybbles) / 2)
      (fun ndx ->
  ...
  let unpack packed =
    let unpacked0 =
      String.init (2 * String.length packed)
        (fun ndx ->
          let byte = Char.code packed.[ndx / 2] in
          (* take high nybble if even ndx, low nybble if odd *)
          if ndx mod 2 = 0 then
            Char.chr ((byte land 0xF0) lsr 4)
          else
            Char.chr (byte land 0x0F))
    in
    ...
    let unpacked1 =
      if flags land 0x02 = 0 then
        ...
        unpacked0 ^ nybble_terminator_string
    in
    let unpacked =
      let len = String.length unpacked1 in
      let odd_len = flags land 0x01 in
      if odd_len = 1 then
        String.sub unpacked1 1 (len - 1)
      else
        String.sub unpacked1 2 (len - 2)
    in
    unpacked
  ...
  let rec get_loop (node : 'a node) (key : Unpacked.t) : 'a option =
    ...
    | BranchNode (subnodes,v) ->
       if key = Unpacked.empty_key then (* at target node *)
         Some v
       else (* get sub node given by first nybble in key *)
...
let make_hash_test s hex =
  Lib.unparse_hex (Data256.to_string (hash_encoded_string s)) = hex
...
let%test "hash_test_2" =
  make_hash_test
    "this is a test of the hash kind"
...
let%test "pack_unpack_1" =
  make_unpack_pack_test "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"

let%test "pack_unpack_2" =
  make_unpack_pack_test "\x05\x05\x05\x05\x05\x05\x05\x05\x05\x05"

let%test "pack_unpack_3" =
  make_unpack_pack_test "\x0A\x0B\x0C\x02"

let%test "pack_unpack_4" =
  make_unpack_pack_test "\x0A\x0F\x0D\x00"

let%test "pack_unpack_5" =
  make_unpack_pack_test "\x0A\x0F\x0E\x0F\x03\x02"

let%test "pack_unpack_5" =
  make_unpack_pack_test ("\x0A\x0F\x0E\x0F\x03" ^ Key.nybble_terminator_string)

let%test "pack_unpack_6" =
  make_unpack_pack_test ""

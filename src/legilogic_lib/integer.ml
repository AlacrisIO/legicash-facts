...
module UIntZ (P : PreUIntZS) = struct
  ...
  let of_0x =
    unary_post_op_check of_0x check_overflow (module_name, "of_hex_string", identity)
  let of_int =
...
module Test = struct
  ...
  let%test "UInt256 <> 0x_string errors" =
    List.for_all
      (fun (hex, err) -> try ignore (UInt256.of_0x hex) ; false with

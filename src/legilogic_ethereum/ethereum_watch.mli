...
val retrieve_relevant_list_logs_data : delay:float -> start_revision:Revision.t -> max_number_iteration:(Revision.t option) -> contract_address:Address.t -> transaction_hash:Digest.t option -> topics:Bytes.t option list -> abi_type list -> abi_value option list -> (Revision.t * (LogObject.t * (abi_value list)) list) Lwt_exn.t

...
type rlp_item =
  | RlpItem of String.t
  | RlpItems of rlp_item list

type t = RlpEncoding of String.t
  [@@deriving show]

(* total length of items to be encoded *)
let rec get_rlp_items_len =
  function
...
let rec encode_int_as_string n =
  if n == 0 then ""
    ...
    (encode_int_as_string (n / 0x100)) ^ (String.make 1 (Char.chr (n mod 0x100)))

(* encode length of string to prepend to RLP-encoding *)
let encode_length len offset =
  if len <= 55 then
    String.make 1 (Char.chr (len + offset))
  else
...
let rec rlp_encode data =
  match data with
  | RlpItem _ -> rlp_encode_item data
  | RlpItems _ -> rlp_encode_items data
...
and rlp_encode_item =
  ...
  | RlpItem item ->
     ...
     if len == 1 && (Char.code (String.get item 0)) <= 0x7F then
       RlpEncoding item
     else
...
let rec decode_int_string n =
  ...
  if len = 0 then 0
    ...
    Char.code n.[len-1] + (decode_int_string (String.sub n 0 (len - 1))) * 0x100

type rlp_length_bounds =
  { start : int
  ; length : int
  }
...
let decode_length input =
  ...
  if len = 0 then
    raise (Internal_error "decode_length: empty input")
  else
    let prefix = Char.code input.[0] in
    if prefix <= 0x7F then
      RlpItemLengthDecoded { start = 0; length = 1 }
      ...
      RlpItemLengthDecoded { start = 1; length = prefix - 0x80 }
              ...
              len > prefix - 0xB7 + decode_int_string(String.sub input 1 (prefix - 0xB7)) then
      ...
      RlpItemsLengthDecoded { start = 1; length = prefix - 0xC0 }
              ...
              len > prefix - 0xF7 + decode_int_string (String.sub input 1 (prefix - 0xF7)) then
      ...
      raise (Internal_error "decode_length: nonconforming RLP encoding")
...
let rlp_decode ((RlpEncoding s) as encoding) =
  ...
  let rec decode_with_leftover s =
    ...
    if len = 0 then (RlpItem "","")
      ...
      | RlpItemLengthDecoded { start; length } ->
         let decoded_len = start + length in (* prefix and data *)
         (RlpItem (String.sub s start length),String.sub s decoded_len (len - decoded_len))
      | RlpItemsLengthDecoded { start; length } ->
         let rec item_loop s0 accum = (* decode items until string consumed *)
           if s0 = "" then
             List.rev accum
           else
         ...
         in
         let decoded_len = start + length in (* prefix and data for all items *)
  ...
  if leftover = "" then decoded
    ...
    raise (Internal_error (Printf.sprintf "For encoding: %s, got leftover data: %s" (show encoding) leftover))
...
let test_int_encoding n =
  decode_int_string (encode_int_as_string n) = n

let%test "int_encoding_inverse_1" =
  test_int_encoding 42

let%test "int_encoding_inverse_2" =
  test_int_encoding 1024

let%test "int_encoding_inverse_3" =
  test_int_encoding 1000000

let%test "int_encoding_inverse_4" =
  test_int_encoding 0

let%test "int_encoding_inverse_5" =
  test_int_encoding 1

(* test that rlp_encode and rlp_decode are inverses *)

let make_rlp_encode_decode_test item =
  rlp_decode (rlp_encode item) = item

let%test "encode_decode_1" =
  make_rlp_encode_decode_test (RlpItem "this is a test")

let%test "encode_decode_2" =
  make_rlp_encode_decode_test
    (RlpItem "a very long string a very long string a very long string a very long string a very long string a very long string a very long string")

let%test "encode_decode_3" =
  make_rlp_encode_decode_test
    (RlpItems [RlpItem "something" ;
               RlpItems [ RlpItem "anything";
                          RlpItems [ RlpItem "une chose quelconque";
                                     RlpItem "un truc" ]]])

let%test "encode_decode_4" =
  make_rlp_encode_decode_test (RlpItems [])

let%test "encode_decode_5" =
  make_rlp_encode_decode_test (RlpItem "")

let%test "encode_decode_6" =
  make_rlp_encode_decode_test
    (RlpItems [RlpItems [RlpItems [RlpItems [RlpItems [RlpItems [RlpItems [RlpItems [RlpItems [ RlpItem "hi, I'm a leaf" ]]]]]]]]])

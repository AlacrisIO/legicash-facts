...
let check_side_chain_contract_created contract_address =
  ...
  if code_red = contract_code_red_sub then
    ...
    (let addr = Address.to_0x contract_address in
     log "Saved contract address %s invalid" addr;
     Printf.eprintf
...
let print_and_retrieve_transaction_hash : Digest.t -> (Address.t * Revision.t) Lwt_exn.t =
  ...
  log "contract_address=%s" (Address.to_0x contract_address);
  log "code_hash=%s" (Digest.to_0x code_hash);
  log "creation_hash=%s" (Digest.to_0x creation_hash);
  log "creation_block=%s" (Revision.to_string creation_block);
  log "E N T R I E S to put in the side_chain_client_config.json file";
  log "  \"contract_address\": \"%s\",\n  \"code_hash\": \"%s\",\n  \"creation_hash\": \"%s\",\n  \"creation_block\": %s," (Address.to_0x contract_address) (Digest.to_0x code_hash) (Digest.to_0x creation_hash) (Revision.to_string creation_block);
  ...
  let%test "deposit_and_payment_and_withdrawal" =
    ...
    Lwt_exn.run
      (fun () ->
        ...
        log "deposit_and_payment_and_withdrawal, step 4";
        ...
        >>= fun () ->
        log "deposit_and_payment_and_withdrawal, step 6";
        let initial_alice_balance = get_alice_balance () in
        let initial_bob_balance = get_bob_balance () in

        (* 1- Test deposit *)
        log "deposit_and_payment_and_withdrawal, step 7";
        ...
        log "deposit_and_payment_and_withdrawal, step 8";
        ...
        (* 2- Test payment *)
        log "deposit_and_payment_and_withdrawal, step 9";
        ...
        log "deposit_and_payment_and_withdrawal, step 10";
        ...
        >>= fun fee_schedule ->
        log "deposit_and_payment_and_withdrawal, step 11";
        ...
        >>= fun (_commitment, _confirmation) ->
        log "deposit_and_payment_and_withdrawal, step 12";

...
let process_request_exn _client_address (in_channel,out_channel) =
  if side_chain_server_log then
    log "process_request_exn, running 1";
  ...
  *)
  >>= fun x ->
  if side_chain_server_log then
    log "Before writing to write_string_to_lwt_io_channel x=(omit)";
...
let process_request client_address channels =
  if side_chain_server_log then
    log "process_request, running 1";
  run_lwt (trying (catching (process_request_exn client_address))
           >>> handling (fun e ->
                   if side_chain_server_log then
                     log "Exception while processing server request: %s" (Printexc.to_string e);
                   return ()))
    channels
...
let _ =
  Lwt_exn.run
    (fun () ->
      ...
      >>= fun () ->
      if side_chain_server_log then
        log "Before the Db.open_connection";
      of_lwt Db.open_connection "alacris_server_db"
      >>= fun () ->
      if side_chain_server_log then
        log "Side_chain_server_config.operator_address=%s" (Address.to_0x Side_chain_server_config.operator_address);
      ...
      if side_chain_server_log then
        log "Using contract %s" (Address.to_0x contract_address);
